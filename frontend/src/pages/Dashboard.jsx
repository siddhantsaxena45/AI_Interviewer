        ...
        <form onSubmit={onSubmit} className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6 items-end">
          ...
          <div className="grid grid-cols-2 gap-4 lg:contents">
            ...
            <div className="space-y-1.5">
              ...
              <select name="duration" value={formData.duration} onChange={onChange} className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-teal-500">
                {DURATIONS.map((dur) => <option key={dur} value={dur}>{dur} Mins</option>)}</select>
            </div>
          </div>
          <div className="space-y-1.5">
